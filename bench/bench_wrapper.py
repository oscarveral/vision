...
def box_filter_benchmark_ffi(image, filter_size, iterations=10):
    ...
    print(f"Box Filter FFI (size={filter_size}) - Mean: {mean_time:.3f} ms, Std Dev: {std_dev:.3f} ms ({iterations} iterations)")
...
def box_filter_benchmark_cv(image, filter_size, iterations=10):
    ...
    mean_time = statistics.mean(times)
    std_dev = statistics.stdev(times) if len(times) > 1 else 0.0
    print(f"Box Filter CV (size={filter_size}) - Mean: {mean_time:.3f} ms, Std Dev: {std_dev:.3f} ms ({iterations} iterations)")
...
def gaussian_filter_benchmark_ffi(image, sigma, iterations=10):
    ...
    mean_time = statistics.mean(times)
    std_dev = statistics.stdev(times) if len(times) > 1 else 0.0
    print(f"Gaussian Filter FFI (sigma={sigma}) - Mean: {mean_time:.3f} ms, Std Dev: {std_dev:.3f} ms ({iterations} iterations)")
...
def gaussian_filter_benchmark_cv(image, sigma, iterations=10):
    ...
    mean_time = statistics.mean(times)
    std_dev = statistics.stdev(times) if len(times) > 1 else 0.0
    print(f"Gaussian Filter CV (sigma={sigma}) - Mean: {mean_time:.3f} ms, Std Dev: {std_dev:.3f} ms ({iterations} iterations)")
