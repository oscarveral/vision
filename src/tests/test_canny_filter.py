...
def test_canny_filter_output_binary():
    ...
    assert all(
        v in [0, 255] for v in unique_values
...
def test_canny_filter_visual_output():
    ...
    for low, high in thresholds:
        edges = canny_edge_detection(
            smoothed, low_threshold=low, high_threshold=high
        )
        ...
        assert (
            edge_pixels > 0
        ), f"Should detect at least some edges (L={low}, H={high})"
...
def test_canny_filter_opencv_comparison():
    ...
    edges_custom = canny_edge_detection(
        smoothed, low_threshold=50, high_threshold=150
    )
    ...
    assert (
        iou > 0.3
    ), f"Edge detection should have reasonable overlap with OpenCV (IoU={iou:.3f})"
