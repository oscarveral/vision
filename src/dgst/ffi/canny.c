...
int32_t canny_edge_detection(const uint8_t* input, uint8_t* output, size_t width, 
    ...
    for (size_t y = 1; y < height - 1; y++) {
        for (size_t x = 1; x < width - 1; x++) {
            ...
            float gx = -(float)input[(y-1) * width + (x-1)] +
                       (float)input[(y-1) * width + (x+1)] -
                       2.0f * (float)input[y * width + (x-1)] +
                       2.0f * (float)input[y * width + (x+1)] -
                       (float)input[(y+1) * width + (x-1)] +
                       (float)input[(y+1) * width + (x+1)];
            
            // Sobel Y (vertical gradient).
            float gy = -(float)input[(y-1) * width + (x-1)] -
                       2.0f * (float)input[(y-1) * width + x] -
                       (float)input[(y-1) * width + (x+1)] +
                       (float)input[(y+1) * width + (x-1)] +
                       2.0f * (float)input[(y+1) * width + x] +
                       (float)input[(y+1) * width + (x+1)];
    ...
    for (size_t y = 1; y < height - 1; y++) {
        for (size_t x = 1; x < width - 1; x++) {
            ...
            if (abs_gx >= abs_gy) {
                ...
                mag1 = (1.0f - weight) * gradient_magnitude[y * width + (x + dx)] +
                       weight * gradient_magnitude[(y + dy) * width + (x + dx)];
                
                // Interpolate in negative direction.
                mag2 = (1.0f - weight) * gradient_magnitude[y * width + (x - dx)] +
                       weight * gradient_magnitude[(y - dy) * width + (x - dx)];
            } else {
                ...
                // Interpolate in positive direction.
                mag1 = (1.0f - weight) * gradient_magnitude[(y + dy) * width + x] +
                       weight * gradient_magnitude[(y + dy) * width + (x + dx)];
                
                // Interpolate in negative direction.
                mag2 = (1.0f - weight) * gradient_magnitude[(y - dy) * width + x] +
                       weight * gradient_magnitude[(y - dy) * width + (x - dx)];
            }
        ...
        for (int32_t dy = -1; dy <= 1; dy++) {
            for (int32_t dx = -1; dx <= 1; dx++) {
                ...
                if (nx > 0 && nx < width - 1 && ny > 0 && ny < height - 1) {
                    if ((float)nms[ny * width + nx] >= low_threshold && 
