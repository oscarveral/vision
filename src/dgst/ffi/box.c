...
int32_t box_filter(const uint8_t* input, uint8_t* output, size_t width, size_t height, size_t filter_size) {
	...
	for (size_t y = 1; y <= height; y++) {
		for (size_t x = 1; x <= width; x++) {
			cum_sum[y * (width + 1) + x] =
			  input[(y - 1) * width + (x - 1)] + cum_sum[(y - 1) * (width + 1) + x] + cum_sum[y * (width + 1) + (x - 1)] - cum_sum[(y - 1) * (width + 1) + (x - 1)];
		}
	}
	...
	for (size_t y = 0; y < height; y++) {
		for (size_t x = 0; x < width; x++) {
			...
			uint32_t sum  = cum_sum[(y2 + 1) * (width + 1) + (x2 + 1)] - cum_sum[(y1) * (width + 1) + (x2 + 1)] - cum_sum[(y2 + 1) * (width + 1) + (x1)] +
